...
def present_dotprobe_stimulus(trial, collect_gaze=True):
    ...
    # Use a circle (dot) instead of a text asterisk
    probe_stim = visual.Circle(
        win=mywin,
        radius=size_fixation_cross_in_pixels / 2,  # radius = half the fixation cross size
        fillColor='black',
        lineColor='black',
        pos=probe_pos
    )
    ...
    while response_clock.getTime() < probe_duration_in_seconds:
        ...
        # check keypress
        keys = kb.getKeys(keyList=response_keys, waitRelease=False)
        if keys:
    ...
    return total_duration, stim_start, face_end, stim_end, probe_onset, gaze_samples, response, rt, correct
...
# --- Bias score storage (for angry trials only) ---
rt_congruent_angry = []
rt_incongruent_angry = []
...
try:
    for trial in trials_handler:
        ...
        stim_duration, stim_start, face_end, stim_end, probe_onset, gaze_samples, response, rt, correct = present_dotprobe_stimulus(trial, collect_gaze=True)

        if rt is not None:
            print(f"Response: {response} | RT: {rt:.3f} s | Correct: {correct}")
            logging.info(f"Response: {response} | RT: {rt:.3f} s | Correct: {correct}")
        else:
            print("No response recorded")
            logging.info("No response recorded")
        ...
        AOI_left,
        AOI_right,
        stim_start,   # face onset
        face_end,     # face offset (used as stim_end inside function)
        probe_onset,
        trial['probe_side']
        )   
        ...
        trials_handler.addData('response_key', response)
        trials_handler.addData('response_rt', rt)
        ...
        if correct == 1 and not trial['filler'] and trial['emo_label'] == 'angry' and rt is not None:
            if trial['congruency'] == 'congruent':
                ...
                rt_incongruent_angry.append(rt)

        # --- Compute running bias score ---
        if len(rt_congruent_angry) > 0 and len(rt_incongruent_angry) > 0:
            mean_cong = sum(rt_congruent_angry) / len(rt_congruent_angry)
            mean_incong = sum(rt_incongruent_angry) / len(rt_incongruent_angry)
            bias_score = mean_incong - mean_cong
        else:
            bias_score = None

        trials_handler.addData('bias_running_angry', bias_score)
