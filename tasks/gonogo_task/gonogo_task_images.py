...
stimulus_duration_in_seconds = 1.3
...
go_image_path = media_folder / "SHINE_color_cielab_1_2.png"
nogo_image_path = media_folder / "SHINE_color_cielab_3.png"

if not go_image_path.exists():
    raise FileNotFoundError(f"Go image not found: {go_image_path}")

if not nogo_image_path.exists():
    raise FileNotFoundError(f"NoGo image not found: {nogo_image_path}")
...
nogo_stimulus = visual.ImageStim(
    win=mywin,
    image=str(nogo_image_path),
    size=stimulus_size,
    pos=(0, 0)
)
...
# PRACTICE SETTINGS
# Phase 1: Go only
N_PRACTICE_PHASE1 = 10
PRACTICE_GO_RATIO = 1.0 # 100% Go

# Phase 2: Mostly Go
N_PRACTICE_PHASE2 = 10
PHASE2_GO_RATIO = 0.7 # 70% Go, 30% NoGo

# Phase 3: Transition ratio
N_PRACTICE_PHASE3 = 10
PHASE3_GO_RATIO = 0.6 # 60% Go, 40% NoGo

N_PRACTICE_TRIALS = N_PRACTICE_PHASE1 + N_PRACTICE_PHASE2 + N_PRACTICE_PHASE3  # 30 total
...
# Performance criterion (checked at end of Phase 3)
PERFORMANCE_CRITERION_ACCURACY = 0.65  # 65% overall correct
MAX_PRACTICE_REPEATS = 2  # max times Phase 2+3 can repeat if criterion not met
...
for phase in phase_handler:
    ...
    if phase == 'baseline':
        ...
        # Format: [str(baseline_counter), 'baseline_start', timestamp]
        ...
        # Format: [str(baseline_counter), 'baseline_end', timestamp]
    ...
    # PRACTICE
        ...
        while not criterion_met and repeat_counter <= MAX_PRACTICE_REPEATS:
            ...
            # --- Build phase trial lists ---
            phase1_list = [{'trial_type': 'go'}] * N_PRACTICE_PHASE1
            phase2_list = ([{'trial_type': 'go'}] * int(N_PRACTICE_PHASE2 * PHASE2_GO_RATIO) +
                           [{'trial_type': 'nogo'}] * int(N_PRACTICE_PHASE2 * (1 - PHASE2_GO_RATIO)))
            phase3_list = ([{'trial_type': 'go'}] * int(N_PRACTICE_PHASE3 * PHASE3_GO_RATIO) +
                           [{'trial_type': 'nogo'}] * int(N_PRACTICE_PHASE3 * (1 - PHASE3_GO_RATIO)))

            # Phase 1 only runs on first attempt
            all_phases = [phase1_list, phase2_list, phase3_list] if repeat_counter == 0 else [phase2_list, phase3_list]

            phase3_accuracy = []  # reset for each attempt

            for practice_trial_list in all_phases:
                ...
                for trial in practice_trials:
                    ...
                    # collect phase 3 accuracy (last 10 trials of the attempt)
                    if practice_trial_list is phase3_list:
                        phase3_accuracy.append(accuracy)
            ...
            # check criterion on phase 3 results 
            n_correct = sum(1 for a in phase3_accuracy if a in ['hit', 'correct_rejection'])
            accuracy_rate = n_correct / len(phase3_accuracy) if phase3_accuracy else 0
            criterion_met = accuracy_rate >= PERFORMANCE_CRITERION_ACCURACY

            print(f"\nPractice accuracy (phase 3): {accuracy_rate:.1%} — {'criterion MET' if criterion_met else 'criterion NOT MET'}")
            logging.info(f"Practice accuracy (phase 3): {accuracy_rate:.1%} — {'MET' if criterion_met else 'NOT MET'}")

            if not criterion_met and repeat_counter < MAX_PRACTICE_REPEATS:
                visual.TextStim(win=mywin, color='white', text="Wir versuchen das nochmal! Press any key when ready.").draw()
                mywin.flip()
                kb.waitKeys()
        ...
        phase_handler.addData('practice_repeats', repeat_counter - 1)
        ...
        # Practice block end trigger
        # Format: ['practice_end', 'go_nogo', timestamp]
    ...
    # MAIN TRIALS
        ...
        # start_time = core.getTime() # was double
        # Main block start trigger
        # Format: ['main_trials_start', 'go_nogo', timestamp]
        ...
        for trial in trials:
            ...
            trial_counter += 1
               ...
               # mid-task break after 100 trials 
            if trial_counter == 100:
                ...
                while core.getTime() - break_start_time < break_duration:
                    ...
                    # --- screen: halfway notice after 60 seconds ---
                    ...
                    # --- screen: 10 second countdown in final 10 seconds ---
                    elif remaining <= 10:
                        visual.TextStim(
                            win=mywin,
                            color='white',
...
# Format: ['experiment_end', 'go_nogo', timestamp]
